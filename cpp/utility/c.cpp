...
void C::setCursor(const int x, const int y) {
#ifdef OS_UNIX
...
void C::write(const std::string &str) {
    printf("%s", str.c_str());
}

void C::writeWithColor(const std::string &str, const ConsoleColor &consoleColor) {
#ifdef OS_UNIX
    int fore = -1, back = -1;
...
WORD C::setColor(const ConsoleColor &consoleColor) {
    WORD color = 0;
    switch (consoleColor.foreColor) {
        case WHITE:
